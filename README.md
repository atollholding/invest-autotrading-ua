...
Після публікації документів через production CMS у `main` GitHub Actions автоматично перегенеровує і коммітить generated manifest, якщо він змінився. Локально цю команду треба запускати вручну тільки після роботи з CMS у local repository mode.
