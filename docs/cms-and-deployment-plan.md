...
- сайт можна перевіряти перед production-релізом без зміни структури контенту.

## Хостинг

Сайт збирається як статичний Astro-проєкт:
...
- branch protection має бути сумісним із прямими CMS-записами в `main`;
- використовувати GitHub CI для `npm run check` і `npm run build`;
...
Шаблон `.github/CODEOWNERS.example` описує рекомендовані межі CODEOWNERS для інженерних файлів.
